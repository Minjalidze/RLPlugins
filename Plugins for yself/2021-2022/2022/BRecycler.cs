...
namespace Oxide.Plugins
{
    ...
    internal class BRecycler : RustLegacyPlugin
    {
        ...
        private void Loaded()
        {
            ...
            recyclerPositions = Core.Interface.GetMod().DataFileSystem.ReadObject<List<string>>("RecyclerPositions");
            recyclers = new List<LootableObject>();

            List<DeployableObject> lootables = UnityEngine.Object.FindObjectsOfType<DeployableObject>().Where(f => IsLootable(f)).ToList();
            if (lootables == null || lootables.Count == 0) return;

            foreach (string recyclerPosition in recyclerPositions)
            {
                Vector3 position = ToVector3(recyclerPosition);
                LootableObject lootable = lootables.Find(f => Vector3.Distance(f.transform.position, position) < 1.0f)?.GetComponent<Inventory>()?.GetLocal<LootableObject>();
                if (lootable == null)
                {
        ...
        private bool IsLootable(DeployableObject deploy) =>
        ...
        private void CMD_SetRecycler(NetUser user, string cmd, string[] args)
        {
            if (user.admin)
            {
                ...
                DeployableObject deployable = obj.GetComponent<DeployableObject>();
                if (deployable == null)
                {
        ...
        private void SaveData() =>
            Core.Interface.GetMod().DataFileSystem.WriteObject("RecyclerPositions", recyclerPositions);
    }
}
