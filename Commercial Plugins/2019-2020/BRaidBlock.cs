...
namespace Oxide.Plugins
{
    ...
    {
        [PluginReference] private Plugin BQuests;
        ...
        private void OnKilled(TakeDamage takeDamage, DamageEvent damage)
        {
            try
            {
                ...
                {
                    var damageUser = damage.attacker.client.netUser;

                    if (takeDamage is HumanBodyTakeDamage)
                    {
                        var victimPlayer = damage.victim.client?.netUser;
                        if (victimPlayer == null || damageUser == victimPlayer) return;
                        Vector3 point;
                        if (IsInRaidZone(damageUser.playerClient.lastKnownPosition, out point)) BQuests?.CallHook("OnKillInBlock", damageUser);
                    }

                    if (damageUser.admin || damage.damageTypes != DamageTypeFlags.damage_explosion ||
                        (!(damage.victim.idMain is DeployableObject) &&
                         !(damage.victim.idMain is StructureComponent)) ||
                        (!(takeDamage is StructureComponentTakeDamage) &&
                         !(takeDamage is ProtectionTakeDamage))) return;
                    var structureComponent = damage.victim.idMain as StructureComponent;
                    var deployableObject = damage.victim.idMain as DeployableObject;
        ...
        private void StartRaidBlock(Vector3 point)
        {
            ...
            {
                if (!blockZones.ContainsKey(point)) return;
                blockZones[point]--;
